...
def mst(distance_matrix):
    ...
    edges = []
    nodes.append(0)
    while len(nodes) < len(distance_matrix):
        minimum_weight = math.inf
        minimum_edge = []
        for node in nodes:
            for i, row in enumerate(distance_matrix):
                if not ((i in nodes) or i == node):
                    if distance_matrix[node][i] < minimum_weight:
                        minimum_weight = distance_matrix[node][i]
                        minimum_edge = [node, i]
...
def nodes_with_odd_degree(T, distance_matrix):
    #iterates through nodes counting edges containing that node
    nodes = []
    for i, row in enumerate(distance_matrix):
        odd_degree = False
        for edge in T:
...
def mwpm(O, distance_matrix):
    #seperating O into seperate distance matrix
    O_distance_matrix = []
    for i, row in enumerate(distance_matrix):
        if i in O:
            O_distance_matrix.append(row)
